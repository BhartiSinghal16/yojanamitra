...
const T = {
  dark: {
    ...
    footerCredit: "rgba(255,255,255,0.2)", schemePillBg: "#1a1a24",
  },
  light: {
    ...
    footerCredit: "rgba(0,0,0,0.25)", schemePillBg: "#ece9e2",
  },
};
...
const ThemeBtn: React.FC<{ theme: Theme; toggle: () => void; t: typeof T.dark }> = ({ theme, toggle, t }) => (
  <button onClick={toggle}
    ...
    style={{ width: 38, height: 38, borderRadius: 10, border: `1px solid ${t.border}`, background: t.bg3, cursor: "pointer", fontSize: 17, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", flexShrink: 0 }}>
    {theme === "dark" ? "☀️" : "🌙"}
  </button>
);
  ...
  <div style={{ display: "flex", background: t.bg3, border: `1px solid ${t.border}`, borderRadius: 10, padding: 3, gap: 2 }}>
    {(["en", "hi"] as Lang[]).map((l) => (
      <button key={l} onClick={() => setLang(l)}
        style={{ border: "none", background: lang === l ? t.gold : "transparent", color: lang === l ? "#0a0a0f" : t.muted, fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 7, cursor: "pointer", transition: "all 0.2s", fontFamily: "'IBM Plex Sans', sans-serif" }}>
        {l === "en" ? "EN" : "हि"}
      </button>
    ))}
  </div>
...
export const Home: React.FC = () => {
  ...
  const [theme, setTheme] = useState<Theme>(() => (localStorage.getItem("ym-theme") as Theme) || "dark");
  ...
  const handleSubmit = async (profile: UserProfile) => {
    ...
    try {
      ...
      console.log("API Response:", data?.total_matched, "schemes found");
      const normalized = {
        session_id: data.session_id || "ai_session",
        total_matched: data.total_matched || data.matches?.length || 0,
  ...
  const globalStyles = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .fade-up-5 { animation: fadeUp 0.65s 0.65s ease both; }
    .cta-btn { font-weight: 700; border: none; cursor: pointer; animation: pulse-ring 2.5s infinite; transition: transform 0.18s, opacity 0.18s; font-family: 'IBM Plex Sans', sans-serif; }
  ...
  const Nav = ({ back }: { back?: Page }) => (
    <nav style={{ borderBottom: `1px solid ${t.border}`, background: t.nav, backdropFilter: "blur(14px)", position: "sticky", top: 0, zIndex: 50 }}>
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: t.bg, minHeight: "100vh", color: t.text, transition: "background 0.3s, color 0.3s" }}>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center", position: "relative" }}>
          ...
          <div className="fade-up-3" style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button className="cta-btn"
              ...
              onClick={() => setPage("form")}>
              {hi ? "📋 प्रोफाइल भरकर खोजें" : "📋 Fill Profile & Match"}
            </button>
            <button
          ...
          {/* OR divider */}
          <div className="fade-up-4" style={{ display: "flex", alignItems: "center", gap: 16, margin: "28px auto", maxWidth: 600 }}>
            <div style={{ flex: 1, height: 1, background: t.border }} />
            <span style={{ fontSize: 12, color: t.muted, fontWeight: 600, padding: "4px 16px", border: `1px solid ${t.border}`, borderRadius: 100, whiteSpace: "nowrap" }}>
              {hi ? "या एक शब्द में खोजें" : "OR search by keyword"}
            </span>
            <div style={{ flex: 1, height: 1, background: t.border }} />
          </div>

          {/* Search bar — in hero */}
          <div className="fade-up-5" style={{ maxWidth: 620, margin: "0 auto", width: "100%", padding: "0 16px" }}>
            <Search theme={theme} language={lang} />
          </div>

          {/* Stats */}
          <div className="stat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginTop: 48, maxWidth: 460, margin: "48px auto 0" }}>
            {[
              { num: "3K+", label: hi ? "योजनाएं" : "Schemes" },
              { num: "15 sec", label: hi ? "में परिणाम" : "Results" },
      ...
      <section style={{ padding: "48px 20px", background: t.bg2, transition: "background 0.3s" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
            {[
              { icon: "🎯", title: hi ? "सटीक मिलान" : "Precise Matching", desc: hi ? "AI-आधारित इंजन — सटीक और पारदर्शी।" : "AI-powered engine — accurate and transparent." },
              ...
              { icon: "🤖", title: hi ? "Groq AI पावर्ड" : "Groq AI Powered", desc: hi ? "हमेशा अपडेटेड — नई योजनाएं भी मिलती हैं।" : "Always updated — finds latest schemes too." },
              { icon: "⚡", title: hi ? "तुरंत परिणाम" : "Instant Results", desc: hi ? "15 सेकंड में पूरी सूची।" : "Full match list in under 15 seconds." },
            ].map((f, i) => (
      ...
      <footer style={{ borderTop: `1px solid ${t.border}`, padding: "28px 20px", textAlign: "center", background: t.bg2, transition: "background 0.3s" }}>
        ...
        <p style={{ color: t.footerCredit, fontSize: 12, marginTop: 4 }}>
          Crafted by <span style={{ color: t.gold }}>Bharti Singhal</span> · Powered by Groq AI
        </p>
      </footer>
    </div>
  );
      ...
      <main style={{ maxWidth: 600, margin: "0 auto", padding: "24px 16px 60px" }}>
        ...
        <div style={{ background: theme === "dark" ? "linear-gradient(135deg, rgba(245,200,66,0.13), rgba(0,212,170,0.07))" : "linear-gradient(135deg, rgba(196,127,10,0.1), rgba(10,140,112,0.06))", border: `1px solid ${t.gold}40`, borderRadius: 20, padding: "20px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: t.muted, marginTop: 2 }}>
              {hi ? "Groq AI द्वारा आपके प्रोफाइल के अनुसार" : "Matched by Groq AI for your exact profile"}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {results?.matches?.map((match: any, i: number) => (
            <SchemeCard key={match.scheme.id || i} match={match} language={lang} rank={i + 1} theme={theme} />
          ))}
        </div>
